...
#import "BKLTTestPlugin.h"
#import "Config.h"
#import "ZmqItem.h"
#define BKLTKEY @"BKLTTest"

@implementation BKLTTestPlugin
{
    BOOL mStopped;
}
...
- (void)executeWithParameters:(NSArray *)parameters
{
    ...
    if(![[[Config instance].testfunctionDir objectForKey:BKLTKEY] containsObject:item.itemMethod])
    {
        return  ;
    }
...
-(void)bklt:(ZmqItem*)item
{
    time_t start,end = 0 ;
    time(&start) ;
    
    
    NSLog(@"BKLTTestPlugin:method:%@ args:%@",item.itemMethod,item.itemArgs) ;
